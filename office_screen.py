...
class OfficeScreen:
        ...
        try:
            if os.path.exists(config_file):
                ...
                with open(config_file, "w", encoding="utf-8") as f:
                    ...
                    self.logger.info(
                        f"Created default configuration file: {config_file}"
                    )
        except json.JSONDecodeError as e:
            self.logger.warning(
                f"Invalid JSON in config file {config_file}: {e}. Using defaults."
            )
        except PermissionError as e:
            self.logger.warning(
                f"Permission denied accessing {config_file}: {e}. Using defaults."
            )
    ...
    def check_memory_usage(self):
        ...
        try:
            ...
            if len(self.performance_metrics["memory_usage"]) > 100:
                self.performance_metrics["memory_usage"] = self.performance_metrics[
                    "memory_usage"
                ][-100:]

            if memory_percent > self.max_memory_usage:
                self.logger.warning(
                    f"High memory usage: {memory_percent:.1f}% ({memory_mb:.1f}MB)"
                )
                return False
    ...
    def adaptive_interval_adjustment(self):
        ...
        if new_interval != self.interval:
            self.logger.info(
                f"Adjusting interval from {self.interval}s to {new_interval:.1f}s (success rate: {success_rate:.2f})"
            )
    ...
    def save_performance_metrics(self):
        ...
        try:
            metrics = {
                ...
                "success_rate": self.successful_visits
                / max(self.total_visits, 1),
                "browser_restarts": self.browser_restart_count,
                "avg_memory_usage": sum(
                    m["percent"] for m in self.performance_metrics["memory_usage"]
                )
        ...
        try:
            ...
            headers = {
                "User-Agent": random.choice(
                    self.config.get(
                        "user_agents",
                        ["Mozilla/5.0 (compatible; OfficeScreen/1.0)"],
                    )
                )
            }
    ...
    def setup_browser(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Could not set window to full screen: {e}. Trying fallback method."
                )
    ...
    def restart_browser(self):
        ...
        self.logger.warning(
            f"Restarting browser (attempt {self.browser_restart_count + 1}/{self.max_browser_restarts})"
        )
        ...
        try:
            ...
            if time.time() - self.last_memory_check > self.memory_check_interval:
                if not self.check_memory_usage():
                    self.logger.warning(
                        "High memory usage detected, restarting browser"
                    )
            ...
            if (
                ...
                or "not found" in current_url.lower()
            ):
            ...
            if any(
                error_indicator in page_source
            ...
            self.performance_metrics["avg_load_time"] = (
                self.performance_metrics["avg_load_time"] * (self.successful_visits - 1)
                + load_time
            ...
            if "chrome not reachable" in str(e).lower() or "session deleted" in str(
                e
            ).lower():
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                self.logger.info(
                    f"Starting cycle {cycle_count} with {len(working_urls)} URLs..."
                )
                ...
                if self.failed_urls:
                    self.logger.warning(
                        f"Failed URLs in this session: {self.failed_urls}"
                    )
    ...
    def load_urls(self, urls_file="config/urls/urls.json"):
        ...
        try:
            if os.path.exists(urls_file):
                ...
                self.logger.warning(
                    f"URLs file {urls_file} not found, using default URLs"
                )
                return self.get_default_urls()
        except json.JSONDecodeError as e:
            self.logger.error(
                f"Invalid JSON in {urls_file}: {e}. Using default URLs."
            )
            return self.get_default_urls()
        except Exception as e:
            self.logger.error(
                f"Failed to load URLs from {urls_file}: {e}. Using default URLs."
            )
            return self.get_default_urls()
